...
const RestaurantDetail = () => {
  ...
  const restaurantData = (data, favorite) => {
    ...
    if (favorite) {
      ...
      favorite.forEach((item) => {
        if (item.positionId === restaurantId) {
    ...
    getWeatherApi(lastDateStr, cardObj.district);
  ...
  React.useEffect(() => {
    ...
    const getTdxId = async () => {
      ...
      Promise.all([
        axios.get(
          `https://tdx.transportdata.tw/api/basic/v2/Tourism/Restaurant?$filter=contains(RestaurantID,'${restaurantId}')&$format=JSON`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          restaurantData(data, favoriteId);
