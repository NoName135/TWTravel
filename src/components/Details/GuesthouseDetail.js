...
const GuesthouseDetail = () => {
  ...
  const filterService = location ? location.filterService : null;
  const filterGrade = location ? location.filterGrade : null;
   ...
   // 取得天氣資料
    ...
    if (favorite) {
      ...
      favorite.forEach((item) => {
        if (item.positionId === guesthouseId) {
    ...
    if (data.Spec) {
      const spec = data.Spec.split(';')
        .map((item) => {
          ...
          if (item.indexOf('房') > -1) {
            return item.split('房')[1] !== '0'
              ? item.split('房').join('房: ')
              : '';
          }
          if (item.indexOf('他') > -1) {
            return item.split('他')[1] !== '0'
              ? item.split('他').join('他: ')
              : '';
          }
        })
        .filter((item) => item);
      cardObj.spec = spec.filter((item, i) => spec.indexOf(item) === i);
      cardObj.spec.sort((a, b) => a.split(': ')[1] - b.split(': ')[1]);
    }

    if (data.ServiceInfo) {
      cardObj.service = data.ServiceInfo.replace(/ <br>/g, '')
        .split(',')
        .filter((item) => item);
    }
    ...
    if (data.Grade) {
      data.Grade = data.Grade.replace('一', '1');
      data.Grade = data.Grade.replace('二', '2');
      data.Grade = data.Grade.replace('三', '3');
      data.Grade = data.Grade.replace('四', '4');
      data.Grade = data.Grade.replace('五', '5');
      cardObj.grade = data.Grade;
    } else {
      cardObj.grade = '';
    }

    const parking = data.ParkingInfo.replace('車位:', '')
      .split('、')
      .filter((item) => item.slice(-3, -1) !== '車0');
    if (parking.length > 0) {
      cardObj.parking = '車位: ' + parking.join('、');
    }

    cardObj.name = data.HotelName;
    cardObj.detail = data.Description;
    cardObj.class = data.Class;
    cardObj.phone = data.Phone;
    cardObj.fax = data.Fax;
  ...
  React.useEffect(() => {
    ...
    const getTdxId = async () => {
      ...
      Promise.all([
        axios.get(
          `https://tdx.transportdata.tw/api/basic/v2/Tourism/Hotel?$filter=contains(HotelID,'${guesthouseId}')&$format=JSON`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          guesthouseData(data, favoriteId);
    ...
    <>
      <div className="container position-relative">
        ...
        <div
          ...
          style={{ paddingTop: '50px', paddingBottom: '15px', zIndex: '1001' }}
        >
          <Link
            to="/guesthouse"
        ...
        <div
          className="d-flex flex-column flex-lg-row justify-content-lg-between align-items-lg-center col-lg-8 mt-5"
        ...
        >
          ...
          {renderData.grade ? (
            <h3
              className="text-center mt-2 mt-lg-0 mb-0 me-lg-2"
              style={{ fontWeight: 'bold' }}
            >
              {renderData.grade}
            </h3>
          ...
          <div className="col-12 col-lg-8">
            ...
            <div className="border border-2 border-primary rounded p-3">
              ...
              <div className="row mt-3">
                <div className="col-12 col-lg-7 tag-border">
                  <h4 className="mb-0" style={{ fontWeight: 'bold' }}>
                    提供服務項目
                  </h4>
                  <div className="d-flex mt-3 text-primary flex-wrap">
                    {!renderData.service ? (
                      <span className="text-dark">無提供服務資訊</span>
                    ) : (
                      renderData.service.map((data, i) => {
                        return (
                          <span
                            className={`border border-primary px-3 py-2 rounded-pill mt-2 ${
                              i !== renderData.service.length - 1 ? 'me-2' : ''
                            }`}
                            key={i}
                          >
                            {data}
                ...
                <div className="col-12 col-lg-5">
                  <h4
                    className="mt-4 mt-lg-0 mb-0"
                    style={{ fontWeight: 'bold' }}
                  >
                    客房價格
                  </h4>
                  <div className="mt-3 d-flex flex-wrap">
                    {!renderData.spec || renderData.spec.length === 0 ? (
                      <span className="text-dark">無提供價格資訊</span>
                    ) : (
                      renderData.spec.map((data, i) => {
                        return (
                          <h5
                            className={`fs-5 pb-2 text-nowrap ${
                              i !== renderData.spec.length - 1 ? 'me-4' : ''
                            }`}
                            key={i}
                          >
                            {data.split(':')[0]}:
          ...
          <div className="col-12 col-lg-4">
            <div className="sticky-top-160">
              ...
              <div className="row row-cols-2 gx-2 gx-lg-3 mt-3">
                <div className="col">
                  <a
                    href={`https://www.google.com.tw/maps/search/景點/@${position[0]},${position[1]},16z`}
                    target="_blank"
                  ...
                  >
                    <div className="btn btn-outline-primary d-flex flex-column justify-content-center align-items-center py-4">
                      <FontAwesomeIcon
                        ...
                        size={'2x'}
                      />
                      <span className="fs-4 pt-2">附近景點</span>
                    </div>
                  </a>
                </div>
                <div className="col">
