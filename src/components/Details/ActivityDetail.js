...
const ActivityDetail = () => {
  ...
  const activityData = (data, favorite) => {
   // 取得天氣資料
    ...
    if (!data.Address) {
      ...
      data.Address.indexOf(data.City) > -1 ||
      data.Location.includes('site') || data.Address === data.Location
    ) {
      cardObj.address = data.Address;
    } else {
      cardObj.address = data.Location.split(' ').join('') + data.Address;
    }
    ...
    cardObj.id = data.ActivityID;
    cardObj.name = data.ActivityName;
    cardObj.detail = data.Description;
    cardObj.address = data.Address;
    const sDate = data.StartTime.split('T')[0];
    const sTime = data.StartTime.split('T')[1].split('+')[0];
    cardObj.startTime = `${sDate} ${sTime}`;
    const eDate = data.EndTime.split('T')[0];
    const eTime = data.EndTime.split('T')[1].split('+')[0];
    cardObj.endTime = `${eDate} ${eTime}`;
    cardObj.cycle = data.Cycle;
  ...
  React.useEffect(() => {
    ...
    const getTdxId = async () => {
      ...
      Promise.all([
        axios.get(
          `https://tdx.transportdata.tw/api/basic/v2/Tourism/Activity?$filter=contains(ActivityID,'${activityId}')&$format=JSON`,
          {
            headers: {
            ...
            <div className="border border-2 border-primary rounded p-3">
              ...
              <p className="d-flex">
                <FontAwesomeIcon
                  ...
                  size={'xl'}
                />
                <span className="ps-2">{renderData.endTime || '全天開放'}</span>
              </p>
              <p className="d-flex">
                <FontAwesomeIcon
                  className="text-primary"
