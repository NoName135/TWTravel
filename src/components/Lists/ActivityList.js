...
import { faSearch, faList, faXmark, faStar, faLocationDot, faDoorOpen, faDoorClosed, faChevronCircleLeft, faChevronCircleRight, faDeleteLeft } from '@fortawesome/free-solid-svg-icons';

import activityTitleImg from '../../img/4.activity.jpg';
...
const ActivityList = () => {
  ...
  // 找出縣市所有類別
  const renderActivityClass = (data) => {
    const activityClassArr = [];
    const classCheck = {};

    data.forEach((obj) => {
      Object.entries(obj).filter((item) => {
        if (item[0].includes('Class')) {
          if (activityClassArr.indexOf(item[1]) === -1) {
            activityClassArr.push(item[1]);
  ...
  // 處理 api 資料
  const activityData = (data, favorite) => {
    const activityArray = [];
    ...
    data.forEach((item) => {
      ...
      cardObj.id = item.ActivityID;
      cardObj.name = item.ActivityName;
      ...
      if (!item.Address) {
        ...
        item.Location.includes('site') || item.Address === item.Location
      ) {
      ...
      const sDate = item.StartTime.split('T')[0];
      const sTime = item.StartTime.split('T')[1].split('+')[0];
      cardObj.startTime = `${sDate} ${sTime}`;
      const eDate = item.EndTime.split('T')[0];
      const eTime = item.EndTime.split('T')[1].split('+')[0];
      cardObj.endTime = `${eDate} ${eTime}`;
      cardObj.cycle = item.Cycle;
      ...
      activityArray.push(cardObj);
    });

    setActivityCards(activityArray);
    //重選縣市時更新資料，沒有就傳送搜尋時的資料
    if (!currentCity || currentCity !== selectCity.value) {
      setInputSearch('');
      setRenderCards(activityArray);
      setPage(
        new Array(Math.ceil(activityArray.length / 27))
          .fill('')
          .map((_, i) => i + 1)
      );
      setTargetPage(1);
      ...
      filterCards(activityArray, location);
    }
  };
    ...
    const getTdx = async () => {
      ...
      Promise.all([
        ...
        .then((res) => {
          ...
          activityData(data, favoriteData);
    ...
    <>
      ...
      <div
        style={{
          ...
          backgroundImage: `url(${activityTitleImg})`,
      ...
      <div className="container mt-5 position-relative">
        {renderCards.length !== 0 ? <h5>共有 {renderCards.length} 處活動</h5> : <h5>搜尋不到任何活動！</h5>}
