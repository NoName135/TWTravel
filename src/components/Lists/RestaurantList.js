...
import { faSearch, faList, faXmark, faStar, faLocationDot, faPhone, faDoorOpen, faChevronCircleLeft, faChevronCircleRight, faDeleteLeft } from '@fortawesome/free-solid-svg-icons';

import restaurantTitleImg from '../../img/2.restaurant.jpg';
...
const RestaurantList = () => {
  ...
  // 找出縣市類別
  const renderRestaurantClass = (data) => {
    const restaurantClassArr = [];
    const classCheck = {};

    data.forEach((obj) => {
      Object.entries(obj).filter((item) => {
        if (item[0].includes('Class')) {
          if (restaurantClassArr.indexOf(item[1]) === -1) {
            restaurantClassArr.push(item[1]);
  ...
  // 處理 api 資料
  const restaurantData = (data, favorite) => {
    ...
    data.forEach((item) => {
      ...
      cardObj.id = item.RestaurantID;
      cardObj.name = item.RestaurantName;
      cardObj.detail = item.Description;
      cardObj.phone = item.Phone;
      cardObj.address = item.Address;
      cardObj.openTime = item.OpenTime?.replace('<p>', '')
        .replace('</p>', '')
        .replace('<br />', ', ')
        .replace('Sun', '週日')
        .replace('Mon', '週一')
      ...
      restaurantArray.push(cardObj);
    });

    setRestaurantCards(restaurantArray);
    //重選縣市時更新資料，沒有就傳送搜尋時的資料
    if (!currentCity || currentCity !== selectCity.value) {
      setInputSearch('');
      setRenderCards(restaurantArray);
      setPage(
        new Array(Math.ceil(restaurantArray.length / 27))
          .fill('')
          .map((_, i) => i + 1)
      );
      setTargetPage(1);
      ...
      filterCards(restaurantArray, location);
    }
  };
  ...
  React.useEffect(() => {
    ...
    const getTdx = async () => {
      ...
      Promise.all([
        await axios.get(
          `https://tdx.transportdata.tw/api/basic/v2/Tourism/Restaurant/${selectCity.value}?$format=JSON`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          restaurantData(data, favoriteData);
    ...
    <>
      ...
      <div
        style={{
          ...
          backgroundImage: `url(${restaurantTitleImg})`,
      ...
      <div className="container mt-5 position-relative">
        {renderCards.length !== 0 ? <h5>共有 {renderCards.length} 處餐廳</h5> : <h5>搜尋不到任何餐廳！</h5>}
      ...
      <nav aria-label="Page navigation font-lilita">
        <ul className="pagination justify-content-center align-items-center fw-bold mt-5">
          ...
          >
            <a className="page-link" href="#/restaurant">
